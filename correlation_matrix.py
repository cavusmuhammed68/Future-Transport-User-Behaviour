...
# Load the dataset
file_path = r'C:\Users\cavus\OneDrive\Masaüstü\Future Transportation\Data for the Future Transportation\station_data_dataverse_Final.csv'
...
sns.heatmap(cm_lr, annot=True, fmt='d', ax=axes[0], cmap='Blues', xticklabels=['Low', 'Medium', 'High'], yticklabels=['Low', 'Medium', 'High'], annot_kws={"size": 15})
...
sns.heatmap(cm_rf, annot=True, fmt='d', ax=axes[1], cmap='Blues', xticklabels=['Low', 'Medium', 'High'], yticklabels=['Low', 'Medium', 'High'], annot_kws={"size": 15})
axes[1].set_title('Random Forest', fontsize=20)
axes[1].set_xlabel('Predicted', fontsize=20)
axes[1].set_ylabel('Actual', fontsize=20)
axes[1].tick_params(axis='both', which='major', labelsize=20)

sns.heatmap(cm_gb, annot=True, fmt='d', ax=axes[2], cmap='Blues', xticklabels=['Low', 'Medium', 'High'], yticklabels=['Low', 'Medium', 'High'], annot_kws={"size": 15})
axes[2].set_title('Gradient Boosting', fontsize=20)
axes[2].set_xlabel('Predicted', fontsize=20)
axes[2].set_ylabel('Actual', fontsize=20)
axes[2].tick_params(axis='both', which='major', labelsize=20)

plt.tight_layout()
plt.savefig('confusion_matrices.png', dpi=1200)
plt.show()
